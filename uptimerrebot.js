...
const myMonitor = new Monitor({
  address:
    "give THe Website Link", //change the ip address of the web to run the bot or leave 127.0.0.1 also known as localhost nha bruh( just for pro 🐧)
  port: 3000, //port bot
  interval: 1, // 10 giây
  config: {
...
myMonitor.on("up", function (res, state) {
  console.log(
    "Check Connected : " + res.address + ":" + res.port + " Is Online !"
  ); //ping
});

myMonitor.on("down", function (res, state) {
  console.log(
    "Check Connected : " + res.address + ":" + res.port + " Is Offline !"
  );
